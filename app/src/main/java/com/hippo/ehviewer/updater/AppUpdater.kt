...
object AppUpdater {
    ...
    /**
     * Manual or auto entry point with UI. Bypasses 1-day throttle when [manualChecking]
     * is true; respects it otherwise. Reentrant — `tryLock` so simultaneous taps no-op.
     */
    fun update(activity: Activity, manualChecking: Boolean) {
        ...
        val scope = (activity as? AppCompatActivity)?.lifecycleScope ?: run {
            Log.w(TAG, "Activity is not AppCompatActivity; cannot launch update")
            return
        }
