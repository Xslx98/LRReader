...
public final class GalleryListScene extends BaseScene
    ...
    public final static int REQUEST_CODE_UPLOAD_ARCHIVE = 1;
...
//        mRecyclerView.setOnGenericMotionListener(this::onGenericMotion);
        ...
        if (LRRAuthManager.getServerUrl() == null) {
            FloatingActionButton fabUpload = mainLayout.findViewById(R.id.fab_upload);
            FloatingActionButton fabUrlDownload = mainLayout.findViewById(R.id.fab_url_download);
    ...
    private ChipGroup buildChipGroup(GalleryInfo gi, ChipGroup tagFlowLayout) {
        ...
        if (null == gi.tgList) {
            String tagName = getString(R.string.lrr_no_preview_tags);
...
//                String[] parts = text.split("(?=(?:(?:[^\"]*\"){2})*[^\"]*$)\\s+");
    ...
    public boolean onItemLongClick(GalleryInfo gi, View view) {
        ...
        textView.setOnClickListener(l -> {
            ...
            Toast toast = Toast.makeText(getEHContext(), R.string.lrr_title_copied, Toast.LENGTH_SHORT);
...
//                .setView(imageViewNew)
                ...
                .setAdapter(new SelectItemWithIconAdapter(context, items, icons), (dialog, which) -> {
                    switch (which) {
                        case 0: // Read
                            Intent intent = GalleryOpenHelper.buildReadIntent(activity, gi);
...
//            mHelper.onGetPageData(taskId, result.pages, result.nextPage, result.galleryInfoList);
     ...
     */
    private void onGetLRRSearchSuccess(LRRSearchResult result, int taskId, int page) {
        if (mHelper != null && mSearchBarMover != null &&
                mHelper.isCurrentTask(taskId)) {
    ...
    class GalleryListHelper extends GalleryInfoContentHelper {
        ...
        private static final int LRR_PAGE_SIZE = 100;
        ...
        protected void getPageData(int taskId, int type, int page) {
            ...
            final String sortBy = mSearchLayout != null ? mSearchLayout.getSortBy() : "date_added";
            final String sortOrder = mSearchLayout != null ? mSearchLayout.getSortOrder() : "desc";

            IoThreadPoolExecutor.Companion.getInstance().execute(() -> {
                try {
                    LRRSearchResult result = (LRRSearchResult) com.hippo.ehviewer.client.lrr.LRRCoroutineHelper.runSuspend(
                            (scope, cont) -> LRRSearchApi.searchArchives(
                                    client, serverUrl, searchFilter, searchCategory,
                                    start, sortBy, sortOrder, false, false, false, cont)
                    );
                    if (getActivity() != null) {
