...
class EhCookieStore(context: Context) : CookieRepository(context, "okhttp3-cookie.db") {
    ...
    fun hasSignedIn(): Boolean {
        val url = EhUrl.HOST_E.toHttpUrlOrNull() ?: return false
    ...
    override fun loadForRequest(url: HttpUrl): List<Cookie> {
        ...
        return if (checkTips) {
            val result = ArrayList<Cookie>(cookies.size + 1)
    ...
    companion object {
        @JvmField
        val KEY_IPD_MEMBER_ID = "ipb_member_id"
        @JvmField
        val KEY_IPD_PASS_HASH = "ipb_pass_hash"
        @JvmField
        val KEY_IGNEOUS = "igneous"

        @JvmField
        val sTipsCookie: Cookie = Cookie.Builder()
